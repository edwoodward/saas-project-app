  ...
  def add_user
    ...
    respond_to do |format|
      if @project_user.save
        format_html { redirect_to users_tenant_project_url(id: @project.id, tenant_id: @project.tenant.id), notice: "user was successfully added to project" }
      else
        format_html { redirect_to users_tenant_project_url(id: @project.id, tenant_id: @project.tenant.id), error: "User was not added to project" }
      end
    end
  end
  ...
  private
    ...
    def verify_tenant
      unless params[:tenant_id] == Tenant.current_tenant_id.to_s
        redirect_to :root, flash: { error: "You are not authorized to access any projects except your own" }
      end
    end
end
